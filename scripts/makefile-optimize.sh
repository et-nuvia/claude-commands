...
section_fix() {
    ...
    for makefile in "${makefiles[@]}"; do
        ...
        if echo "$content" | grep -q 'FORMAT ?= human' && ! echo "$content" | grep -q 'MAKEFLAGS.*--no-print-directory'; then
            local temp_file
            temp_file=$(mktemp)
            awk '{print; if ($0 ~ /FORMAT \?= human/) print "MAKEFLAGS += --no-print-directory"}' "$makefile" > "$temp_file"
            mv "$temp_file" "$makefile"
            modified=true
            fixes_applied=$((fixes_applied + 1))
        ...
        if echo "$content" | grep -q 'FORMAT ?= human' && ! echo "$content" | grep -q 'JSON_WRAPPER'; then
            local temp_file
            temp_file=$(mktemp)
            awk '{print; if ($0 ~ /MAKEFLAGS.*--no-print-directory/) print "JSON_WRAPPER ?= $(HOME)/.claude/scripts/lib/make-json-wrapper.sh"}' "$makefile" > "$temp_file"
            mv "$temp_file" "$makefile"
            modified=true
            fixes_applied=$((fixes_applied + 1))
