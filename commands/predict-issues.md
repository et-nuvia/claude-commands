...
```bash
~/.claude/scripts/track-command.sh --command "predict-issues" --event start
```

If the workflow encounters an unrecoverable error at any point, run:
```bash
~/.claude/scripts/track-command.sh --command "predict-issues" --event error \\
  --model "MODEL_ID" \\
...
```bash
~/.claude/scripts/track-command.sh --command "predict-issues" --event complete \
  --model "MODEL_ID" \
